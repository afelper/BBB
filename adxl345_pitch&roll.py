...
if (acceleration_x & (1<<16-1)):
    acceleration_x = acceleration_x - (1 << 16)
...
if (acceleration_z & (1<<16-1)):
    acceleration_z = acceleration_z - (1 << 16)

factor_gravedad = 4.0 #por defecto para +/- 2g
resolucion = 1024.0  # resolucion 10 bits
...
# Calcular el pitch (inclinación hacia adelante/atrás)
pitch = math.atan2(acceleration_y, math.sqrt(acceleration_x**2 + acceleration_z**2))

# Calcular el roll (balance hacia la izquierda/derecha)
roll = math.atan2(acceleration_x, math.sqrt(acceleration_y**2 + acceleration_z**2))
...
print(f"Roll en grados: {roll_degrees: .3f}")
print(f"Pitch en grados: {pitch_degrees: .3f}")
